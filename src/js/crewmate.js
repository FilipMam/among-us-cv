class Crewmate {
        ...
        if (!this.state.movingInterval) {
            this.state.movingInterval = setInterval(() => {
                ...
                if (this.state.moving.left) {
                    ...
                    if (
                        newPosX > boundries.left &&
                        !this.willHitObstacle(
                            newPosX,
                            newPosX + box.width,
                            positionBottom,
                            box.height
                ...
                if (this.state.moving.up) {
                    let newPosY =
                        positionBottom - this.state.marginBottom / 2 - pacePXL;
                    if (
                        newPosY > boundries.top &&
                        !this.willHitObstacle(
                            box.left,
                            box.right,
                            newPosY,
    ...
    stopMoving = (dir) => {
        ...
        if (!Object.entries(this.state.moving).some((entry) => entry[1])) {
            ...
            this.element.classList.remove(
                `moving--${this.state.movingAnimtationFrame}`
            );
    ...
    willHitObstacle = (xL, xR, y) => {
        return (
            this.shipManager.obstacles.filter((obstacle) =>
                obstacle.willBeHitByCrewmate(
                    xL,
                    xR,
                    y,
                    y - this.state.marginBottom
                )
            ).length > 0
        );
    };
    ...
    updateAnimationFrame = () => {
        ...
        this.state.movingAnimtationFrame =
            this.state.movingAnimtationFrame === 12
                ? 1
                : this.state.movingAnimtationFrame + 1;
